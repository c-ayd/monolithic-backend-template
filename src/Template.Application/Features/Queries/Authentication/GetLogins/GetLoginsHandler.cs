...
namespace Template.Application.Features.Queries.Authentication.GetLogins
{
    public class GetLoginsHandler : IRequestHandler<GetLoginsRequest, ExecResult<GetLoginsResponse>>
    {
        ...
        private readonly IFlexLogger<GetLoginsHandler> _flexLogger;

        public GetLoginsHandler(IUnitOfWork unitOfWork,
            IRequestContext requestContext,
            IHashing hashing,
            IOptions<AccountLockSettings> accountLockSettings,
            IFlexLogger<GetLoginsHandler> flexLogger)
        {
        ...
        public async Task<ExecResult<GetLoginsResponse>> Handle(GetLoginsRequest request, CancellationToken cancellationToken)
        {
            ...
            // Get logins
            var logins = await _unitOfWork.Logins.GetAllActiveByUserIdAsync(_requestContext.UserId!.Value);
